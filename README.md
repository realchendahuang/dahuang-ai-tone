...
1. **加味**（主）——把一段普通文本改写成某个 AI 模型的风格。改的是句子组织、语气、行为模式，不是堆口癖。
   ...
   - 也可以指定版本：Claude 4.8 的直球味（当前默认）、Claude 3.x 的谨慎味（历史版本）、o-series 的无 markdown 味，等等。
...
**加 Gemini 味**（夸夸开头、类比、引号癖）：
> 好问题！这个功能现在不适合直接上线。你可以把它想象成盖房子——用户需求是"地基"，地基没打牢就往上盖，返工是迟早的事。先做 3-5 个用户访谈，把"真实需求"摸清楚，再排期也不迟。
...
| Anthropic Claude | 4.8 直给观点不啰嗦（当前）；3.x 温柔谨慎 hedging（历史味） |
...
│   ├── reverse-humanizer.md  # 加味总方法 + 交付格式 + 自检规则
│   ├── common-ai-patterns.md # 跨风格通用 AI 味清单（21 项）
